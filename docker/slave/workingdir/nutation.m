...
%--------------------------------------------------------------------
% nutation function     Calculate the Nutation in longitude, latitude
%                     and nutation matrix.
% Input  : - Column vector of julian day.
%          - Type of nutation matrix:
%            'f' : full precision, (default).
%            'l' : linearized matrix.
% Output : - Matrix of nutation. (line per JD).
%            First column for the nutation in longitude.
%            Second column for nutation in obliquity. (radians).
...
%    URL : htpp://wise-obs.tau.ac.il/~eran/matlab.html
%--------------------------------------------------------------------
if (nargin==1),
   MatType = 'f';
elseif (nargin==2),
